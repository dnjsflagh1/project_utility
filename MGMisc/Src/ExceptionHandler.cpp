...
#define XCRASHREPORT_ERROR_LOG_FILE		_T("log.log")
#define XCRASHREPORT_CRASH_REPORT_APP	_T("ErrReporter.exe")
...
#define STATUS_ILLEGAL_VLM_REFERENCE     ((DWORD   )0xC00002C0L)     
		...
		LPCTSTR lpszMessage)
	{
		...
		SYSTEMTIME date;
		GetLocalTime(&date) ;
		MGStrOp::sprintf(tempFile,sizeof(tempFile) -1,"%s%02d_%02d_%02d_%d_%d_%d_%s",DmpPath,date.wYear, date.wMonth, date.wDay,date.wHour,date.wMinute,date.wSecond,XCRASHREPORT_ERROR_LOG_FILE);
		lstrcpy(pszFilePart, XCRASHREPORT_ERROR_LOG_FILE);
		
		HANDLE hLogFile = CreateFile(tempFile, GENERIC_WRITE, 0, 0,
		...
		// Create the file
		ZeroMemory(tempFile, sizeof(tempFile));
		MGStrOp::sprintf(tempFile,sizeof(tempFile) -1,"%s%02d_%02d_%02d_%d_%d_%d_%s",DmpPath,date.wYear, date.wMonth, date.wDay,date.wHour,date.wMinute,date.wSecond,XCRASHREPORT_MINI_DUMP_FILE);
		...
		else
		{
			...
			lstrcat(szCommandLine, _T(" /i:"));
			lstrcat(szCommandLine, g_ErrIP);
			lstrcat(szCommandLine, _T(";"));
			lstrcat(szCommandLine, _T("/p:"));
			lstrcat(szCommandLine, g_ErrPort);
			lstrcat(szCommandLine, _T(";"));
			lstrcat(szCommandLine, _T("/v:"));
			lstrcat(szCommandLine, g_ErrVer);
			lstrcat(szCommandLine, _T(";"));
			int result = WinExec(szCommandLine, SW_SHOW);
			if (result>31)
			{
				MessageOut(szCommandLine);
				MessageOut("\nSend ErrReport OK! \n");
