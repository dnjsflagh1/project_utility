...
//---------------------------------------------------------------------------------------------------------
	//MD5摘要
	MD5VAL MGMd5::md5(char * str, unsigned int size)
	{
		if(size==0)
			size=strlen(str);
...
#define BUFFER_SIZE 4096   //必须是64的倍数
	...
	//MD5文件摘要
	MD5VAL MGMd5::md5File(FILE * fpin)
	{
		if(!Buffer)
	...
	//---------------------------------------------------------------------------------------------------------
	std::string MGMd5::md5Str( char * str, unsigned int size/*=0*/ )
	{
		std::string returnMD5Str;
		char returnMD5Strtemp[1024];
		MD5VAL val = md5(str,size);
		wsprintf(returnMD5Strtemp, "%08x%08x%08x%08x", conv(val.a),conv(val.b),conv(val.c),conv(val.d));
		returnMD5Str = returnMD5Strtemp;
		return returnMD5Str;
	}
	//---------------------------------------------------------------------------------------------------------
	std::string MGMd5::md5FileStr( FILE * fpin )
	{
		std::string returnMD5Str;
		char returnMD5Strtemp[1024];
		MD5VAL val = md5File(fpin);
		wsprintf(returnMD5Strtemp, "%08x%08x%08x%08x", conv(val.a),conv(val.b),conv(val.c),conv(val.d));
		returnMD5Str = returnMD5Strtemp;
		return returnMD5Str;
	}
	//---------------------------------------------------------------------------------------------------------
	unsigned int MGMd5::conv( unsigned int a )
	{
		unsigned int b=0;
		b|=(a<<24)&0xff000000;
		b|=(a<<8)&0x00ff0000;
